...
class FixedExperienceMaker(ABC):
    """
    ...
    def __init__(
        ...
        super().__init__()
        self.actor = actor
        self.llm = LLM(model="Qwen/Qwen2.5-3B-Instruct", max_num_seqs=1, task="generate")
    ...
    def train_reward_model(self, concatenated_summaries, concatenated_train_prompts, concatenated_train_rejected, concatenated_train_chosen):
        ...
        for start_idx in range(0, total_samples, batch_size):
            ...
            summaries, chosen, rejected, reward_prompts = concatenated_summaries[start_idx: end_idx], concatenated_train_chosen[start_idx: end_idx], concatenated_train_rejected[start_idx: end_idx], concatenated_train_prompts[start_idx: end_idx]
            # print(summaries)
            # prompt_delimiter = "Do not focus on the topics of specific conversations. Keep your summary concise—under 50 words.assistant"
            # summaries = [summary.split(prompt_delimiter)[1].strip() for summary in summaries]
            summaries = [summary.strip() for summary in summaries] 
            chosen_chats = []
            for (p, reward_prompt) in enumerate(reward_prompts):
                user_summary = summaries[p]
                cur_prompt = f"User: Here is the summary of my preferences based on previous conversations: {user_summary} In the following conversation, keep this in mind to personalize your response.\n\n" + reward_prompt 
                parts = re.split(r'(User:|Assistant:)', cur_prompt)
                chat = [ ] # {"role": "system", "content": f"You are a helpful assistant that personalizes responses based on the user's preferences. Here is the summary of the user's preferences, generated by another AI assistant based on previous conversations: {user_summary}"}]
                for i in range(1, len(parts), 2):
                    role = parts[i].lower().replace(":", "").strip()  # "user" or "assistant"
                    content = parts[i + 1].strip()
                    chat.append({"role": role, "content": content})
                chat.append({"role": "assistant", "content": chosen[p]})
                chosen_chats.append(deepcopy(chat))
            ...
            for (p, reward_prompt) in enumerate(reward_prompts):
                ...
                for i in range(1, len(parts), 2):
                    ...
                    content = parts[i + 1].strip()
                    chat.append({"role": role, "content": content})
                chat.append({"role": "assistant", "content": rejected[p]})
                rejected_chats.append(deepcopy(chat))
            
            
            # formatted_chosen = []
            # formatted_rejected = []
            # for chat in chosen_chats:
            #    tmp = [x["role"] + ": " + x["content"].strip() for x in chat]
            #    formatted_chosen.append("\n".join(tmp))
            # for chat in rejected_chats:
            #    tmp = [x["role"] + ": " + x["content"].strip() for x in chat]
            #    formatted_rejected.append("\n".join(tmp))
            
            formatted_chosen = [self.reward_tokenizer.apply_chat_template(chat, tokenize=False, add_generation_prompt=False) for chat in chosen_chats]
            formatted_rejected = [self.reward_tokenizer.apply_chat_template(chat, tokenize=False, add_generation_prompt=False) for chat in rejected_chats]
            self.reward_tokenizer.pad_token = self.reward_tokenizer.eos_token
            # Tokenization with padding and truncation
            all_texts = formatted_chosen + formatted_rejected
            all_tokens = self.reward_tokenizer(
                all_texts,
                max_length=16384,
                padding=True,
                truncation=True,
            ...
            batch_size = len(formatted_chosen)
            chosen_tokens = {k: v[:batch_size] for k, v in all_tokens.items()}
            rejected_tokens = {k: v[batch_size:] for k, v in all_tokens.items()}
    ...
    def evaluate_reward_model(self, summaries, prompts, rejected, chosen, return_all=False):
        ...
        chosen_chats = []
        for (p, reward_prompt) in enumerate(prompts):
            user_summary = summaries[p]
            cur_prompt = f"User: Here is the summary of my preferences based on previous conversations: {user_summary} In the following conversation, keep this in mind to personalize your response.\n\n" + reward_prompt 
            # cur_prompt = "User: " + reward_prompt 
            parts = re.split(r'(User:|Assistant:)', cur_prompt)
        ...
        # change to accuracy
        if return_all:
            acc = (chosen_r > rejected_r).float().detach().cpu().numpy()
            rew = np.array([- self.prediction_reward_func(c, r).detach().float().item() for c, r in zip(chosen_r, rejected_r)])
            print(acc, rew)
            self.fin_eval_accs.append(acc)
            self.fin_eval_rews.append(rew)
            return 
        acc = (chosen_r > rejected_r).float().mean().item()
        r = np.mean([- self.prediction_reward_func(c, r).float().item() for c, r in zip(chosen_r, rejected_r)])
        self.eval_accs.append(acc)
        self.eval_rews.append(r)
    ...
    def compute_reward_model(self, experiences):
        for experience in experiences:
            ...
            for (p, reward_prompt) in enumerate(reward_prompts):
                ...
                chat.append({"role": "assistant", "content": chosen[p]})
                chosen_chats.append(deepcopy(chat))
            print("<CHOSEN CHAT/>", chosen_chats[-1], "</CHOSEN CHAT>\n")
            rejected_chats = []
            for (p, reward_prompt) in enumerate(reward_prompts):
                user_summary = summaries[p]
                cur_prompt = f"User: Here is the summary of my preferences based on previous conversations: {user_summary} In the following conversation, keep this in mind to personalize your response.\n\n" + reward_prompt 
    ...
    def make_experience(self, samples: Samples) -> Experience:
        ...
        # prompt_input = self.tokenize_fn(prompts, self.prompt_max_len, device="cuda")
        # prompt_input_ids = prompt_input["input_ids"]
        # seq_lens = prompt_input_ids.size(1)
        # # Compute prompt lengths (exclude padding)
        # prompt_lens = prompt_input_ids.ne(self.tokenizer.pad_token_id).sum(dim=1).tolist()
        # prompt_start_idxs = [seq_lens - l for l in prompt_lens]
        # sequences_cpu = sequences.cpu()

        # # Slice out only the model-generated part
        # response_only = [
        #     seq[start_idx:] for seq, start_idx in zip(sequences_cpu, prompt_start_idxs)
        # ]

        # Generate the summaries using vllm (default sampling params)
        print("==== Inside fixed experience maker ====")
        print(f"PROMPTS: {prompts}\n\n")
        outputs = [self.llm.generate(prompt, sampling_params=self.sampling_params) for prompt in prompts]
        print(outputs,"\n\n")
        summaries = [output[0].outputs[0].text for output in outputs]
        print(f"SUMMARIES: {summaries}\n\n")

        # summaries = self.tokenizer.batch_decode(sequences.cpu(), skip_special_tokens=True)
        # summaries = [summary.split("Summary##")[-1].strip() for summary in summaries]
        summaries = [summary.split("##Summary")[-1].strip().removeprefix("assistant").strip() for summary in summaries]

        print("\n<MAKE EXPERIENCE>", "==".join(summaries), "<MAKE EXPERIENCE>\n")
