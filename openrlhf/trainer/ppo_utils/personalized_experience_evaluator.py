...
class PersonalizedExperienceEvaluator(ABC):
    ...
    def compute_reward_model(self, experiences):
        for experience in experiences:
            ...
            # chosen_chats = ["<|im_start|>user\n" + "Based on the following summary about the user from their past conversation, respond to the user's request that aligns with the user's preference. Summary: " + summary + "\nStart of a new conversation: " + prompt + "<|im_end|>\n<|im_start|>assistant\n" + response + "<|im_end|>" + " " + self.reward_tokenizer.eos_token for summary, response, prompt in zip(summaries, samples.chosen, samples.reward_prompts)]
            # rejected_chats = ["<|im_start|>user\n" + "Based on the following summary about the user from their past conversation, respond to the user's request that aligns with the user's preference. Summary: " + summary + "\nStart of a new conversation: " + prompt + "<|im_end|>\n<|im_start|>assistant\n" + response + "<|im_end|>" + " " + self.reward_tokenizer.eos_token for summary, response, prompt in zip(summaries, samples.rejected, samples.reward_prompts)]
    ...
    def make_experience_list(
        ...
        for experience, reward in zip(experiences, rewards):
            ...
            # self.summary_logs.append(experience.info["summary"])
            experience.kl = None
            del experience.info["num_actions"]
            # del experience.info["summary"]
            del experience.info["chosen"]
            del experience.info["rejected"]
            # del experience.info["train_rejected"]
            # del experience.info["train_chosen"]
            # del experience.info["eval_rejected"]
            # del experience.info["eval_chosen"]
            # del experience.info["reward_prompt"]
