...
from .ppo_utils import AdaptiveKLController, Experience, FixedKLController, NaiveExperienceMaker, NaiveReplayBuffer, PersonalizedExperienceMaker
...
class PPOTrainer(ABC):
    ...
    def __init__(
        ...
        **generate_kwargs,
        ...
        self._train_reward = self.args.train_reward
        # Mixtral 8x7b
        ...
        print(f"***Currently specifying reward tokenizer to be {self.args.reward_pretrain} in PPO trainer***")
        # self.reward_tokenizer = tokenizer
        self.reward_tokenizer = AutoTokenizer.from_pretrained(self.args.reward_pretrain, trust_remote_code=True)
        self.experience_maker = PersonalizedExperienceMaker(
            actor,
            critic,
            reward_model,
            initial_model,
    ...
    def fit(
        ...
        if eval_dataloader is not None:
            ...
            self.eval_dataloader = prompts_dataloader
        ...
        for episode in range(start_episode, args.num_episodes):
            ...
            for rand_prompts, rejected, chosen, reward_prompts, train_prompts, eval_prompts, train_rejected, train_chosen, eval_rejected, eval_chosen in self.prompts_dataloader:
                concatenated_summaries = []
                # for training rm
                concatenated_train_prompts = []
                concatenated_train_rejected = []
                concatenated_train_chosen = []
                # for evaluating rm
                concatenated_eval_prompts = []
                concatenated_eval_rejected = []
                concatenated_eval_chosen = []
                for i, experience in enumerate(
                    self.experience_maker.make_experience_list(rand_prompts, rejected, chosen, reward_prompts, train_prompts, eval_prompts, train_rejected, train_chosen, eval_rejected, eval_chosen, **self.generate_kwargs)
                ):
                    # if i == 0:
                    ...
                    save_path = os.path.join("results/" + self.debug_log_path)
                    concatenated_summaries.append(experience.info["summary"])
                    if self._train_reward:
                        concatenated_train_prompts.append(experience.info["train_prompt"])
                        concatenated_train_rejected.append(experience.info["train_rejected"])
                        concatenated_train_chosen.append(experience.info["train_chosen"])
                    # for rm eval
                    concatenated_eval_prompts.append(experience.info["eval_prompt"])
                    concatenated_eval_rejected.append(experience.info["eval_rejected"])
                    concatenated_eval_chosen.append(experience.info["eval_chosen"])
                ...
                # evaluate reward model acc 
                flat_summaries = [item for sublist in concatenated_summaries for item in sublist]
                flat_prompts = [item for sublist in concatenated_eval_prompts for item in sublist]
                flat_rejected = [item for sublist in concatenated_eval_rejected for item in sublist]
                flat_chosen = [item for sublist in concatenated_eval_chosen for item in sublist]
                # # this runs fine but not when it's run outside? 
                # self.experience_maker.evaluate_reward_model(flat_summaries, flat_prompts, flat_rejected, flat_chosen)
                # step_list.append(steps)
                # episode_list.append(episode)
                # # add reward model acc
                # self.experience_maker.summary_logs.append(deepcopy(flat_summaries))
                # pickle.dump({"eval_acc": self.experience_maker.eval_accs, "eval_rew": self.experience_maker.eval_rews, "summary": self.experience_maker.summary_logs, "rew": self.experience_maker.prediction_rew_logs, "acc": self.experience_maker.prediction_acc_logs, "episode": episode_list, "step": step_list}, open(save_path, "wb"))
                # print("RM eval acc: ", self.experience_maker.eval_accs[-1], "\n", "RM eval rew:", self.experience_maker.eval_rews[-1], "\n\n")
                # train reward
                if self._train_reward:
                    flat_prompts = [item for sublist in concatenated_train_prompts for item in sublist]
                    flat_rejected = [item for sublist in concatenated_train_rejected for item in sublist]
                    flat_chosen = [item for sublist in concatenated_train_chosen for item in sublist]
                    self.experience_maker.train_reward_model(flat_summaries, flat_prompts, flat_rejected, flat_chosen)
            ...
            # save rm, actor, critic
            model_save_path = os.path.join("ppo_models/eps" + str(episode) + self.model_save_path)
            self.strategy.save_model(self.actor, self.tokenizer, model_save_path + "_actor")
            self.strategy.save_model(self.critic, self.tokenizer, model_save_path + "_critic")
            self.strategy.save_model(self.reward_model, self.reward_tokenizer, model_save_path + "_reward")
    ...
    def _save_checkpoint(self, args, tag, client_states):
        ...
        if self.save_hf_ckpt:
            ...
            self.strategy.save_model(self.reward_model, self.reward_tokenizer, os.path.join(args.ckpt_path, f"{tag}_reward"))
