...
from openrlhf.models import InfoNCELoss, SAMILoss
from openrlhf.models.utils import log_probs_from_logits
from openrlhf.utils.distributed_sampler import DistributedSampler


class InfoNCETrainer(ABC):
    """
    ...
    def __init__(
        ...
        self.beta = beta
        if self.sami_loss:
            self.loss_fn = SAMILoss()
        else:
            self.loss_fn = InfoNCELoss(self.beta, self.args.label_smoothing, self.args.ipo)
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                if not self.packing_samples:
                    # now need to do chosen-swapped context, rejected-swapped context
                    ...
                    packed_input_ids, packed_attention_masks, packed_seq_lens, swap_packed_input_ids, swap_packed_attention_masks, swap_packed_seq_lens, prompt_id_lens, swap_prompt_id_lens = data
                    # chosen-rejected response for chosen prompt
                    packed_input_ids, packed_attention_masks = packed_input_ids.to(
                        torch.cuda.current_device()
                    ), packed_attention_masks.to(torch.cuda.current_device())

                    # chosen-rejected response for rejected prompt
                    swap_packed_input_ids, swap_packed_attention_masks = swap_packed_input_ids.to(
                        torch.cuda.current_device()
                    ), swap_packed_attention_masks.to(torch.cuda.current_device())

                    chosen_chosen_logps, chosen_rejected_logps, aux_loss, nll_loss = self.packed_samples_forward(
                        self.model, packed_input_ids, packed_attention_masks, packed_seq_lens, prompt_id_lens
                    )
                    
                    rejected_chosen_logps, rejected_rejected_logps, aux_loss, nll_loss = self.packed_samples_forward(
                        self.model, swap_packed_input_ids, swap_packed_attention_masks, swap_packed_seq_lens, swap_prompt_id_lens
                    )
                    # infoNCE doesn't use the same DPO loss with KL constraint
                    # with torch.no_grad():
                ...
                # loss function
                preference_loss = self.loss_fn(
                    chosen_chosen_logps, chosen_rejected_logps, rejected_chosen_logps, rejected_rejected_logps
                )
                # mixtral
                ...
                acc = 0. # ignore this value
                acc_sum += acc
                loss_sum += preference_loss.item()
                # dpo logs
                logs_dict = {
                    "loss": preference_loss.item(),
                    "acc": acc, # ignore this term
                    "chosen_reward": 0, # ignor this, chosen_reward.mean().item(),
                    "reject_reward": 0, # ignore this, reject_reward.mean().item(),
        ...
        with torch.no_grad():
            ...
            for data in eval_dataloader:
                if not self.packing_samples:
                    ...
                    chosen_chosen_logps, chosen_rejected_logps, aux_loss, _ = self.packed_samples_forward(
                        self.model, packed_input_ids, packed_attention_masks, packed_seq_lens, prompt_id_lens
                    )
                    
                    rejected_chosen_logps, rejected_rejected_logps, aux_loss, _ = self.packed_samples_forward(
                        self.model, swap_packed_input_ids, swap_packed_attention_masks, swap_packed_seq_lens, swap_prompt_id_lens
                    )
                ...
                acc_sum += 0 # ignore acc in infoNCE trainer
                loss_sum += loss.item()
                times += 1
                step_bar.update()
