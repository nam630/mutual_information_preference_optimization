...
from openrlhf.models import BinaryLoss, LogExpLoss, PairWiseLoss, RegressionLoss
...
class RewardModelTrainer(ABC):
    ...
    def __init__(
        ...
        if loss == "sigmoid":
            ...
            self.strategy.print("Regression Loss")
        elif loss == "classification":
            self.loss_fn = BinaryLoss()
            self.strategy.print("Classification loss on binary labels")
        else:
    ...
    def fit_multi_headed_classification(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                if not self.packing_samples:
                    chosen_ids, c_mask, repetition_rewards, positive_rewards, negative_rewards = data
                    # print(chosen_ids, c_mask, repetition_rewards, positive_rewards, negative_rewards, "\n\nLine 137 in rm_trainer")
                    chosen_ids = chosen_ids.squeeze(1).to(torch.cuda.current_device())
                    c_mask = c_mask.squeeze(1).to(torch.cuda.current_device())
                    # should be outputted as a batch [batch, 3]
                    all_values, _ = self.model(chosen_ids, attention_mask=c_mask, return_output=True)

                else:
                    packed_input_ids, packed_attention_masks, packed_seq_lens, repetition_rewards, positive_rewards, negative_rewards = data
                    # how to combine the model output and interpret
                    packed_input_ids, packed_attention_masks = packed_input_ids.to(
                        torch.cuda.current_device()
                    ), packed_attention_masks.to(torch.cuda.current_device())
                    # print(repetition_rewards, positive_rewards, negative_rewards, "\n\nLine 145 in rm_trainer")
                    all_values, output = self.model(
                        packed_input_ids,
                ...
                rewards = torch.tensor([repetition_rewards, positive_rewards, negative_rewards]).T 
                # print(rewards.shape, "Line 158")
                ...
                classification_loss = self.loss_fn(all_values, rewards)
                # mixtral
                if not self.aux_loss:
                    aux_loss = 0

                loss = classification_loss + aux_loss * self.args.aux_loss_coef
                self.strategy.backward(loss, self.model, self.optimizer)
                self.strategy.optimizer_step(self.optimizer, self.model, self.scheduler)

                loss_sum += classification_loss.item()
                # optional rm info
                logs_dict = {
    ...
    def fit_regression(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        # get eval and save steps
        if args.eval_steps == -1:
            args.eval_steps = num_update_steps_per_epoch  # Evaluate once per epoch
        if args.save_steps == -1:
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                if not self.packing_samples:
                    chosen_ids, c_mask, rewards = data
                    chosen_ids = chosen_ids.squeeze(1).to(torch.cuda.current_device())
                    c_mask = c_mask.squeeze(1).to(torch.cuda.current_device())
                    all_values, _ = self.model(chosen_ids, attention_mask=c_mask, return_output=True)

                else:
                    packed_input_ids, packed_attention_masks, packed_seq_lens, rewards = data
                    packed_input_ids, packed_attention_masks = packed_input_ids.to(
                        torch.cuda.current_device()
                    ), packed_attention_masks.to(torch.cuda.current_device())
                    all_values, output = self.model(
                ...
                regression_loss = self.loss_fn(all_values, rewards)
                # mixtral
                if not self.aux_loss:
                    aux_loss = 0

                loss = regression_loss + aux_loss * self.args.aux_loss_coef
                self.strategy.backward(loss, self.model, self.optimizer)
                self.strategy.optimizer_step(self.optimizer, self.model, self.scheduler)

                loss_sum += regression_loss.item()
                # optional rm info
                logs_dict = {
    ...
    def final_evaluate(self, eval_dataloader, steps=0):
        self.model.eval()
        with torch.no_grad():
            acc = 0
            accs = []
            ...
            for data in eval_dataloader:
                ...
                rewards += [chosen_reward.float().mean().item(), reject_reward.float().mean().item()]
                acc += (chosen_reward > reject_reward).float().mean().item()
                accs.append((chosen_reward > reject_reward).float().mean().item())
                loss_sum += loss.item()

            acc_mean = acc / self.eval_dataloader.__len__()
            loss_mean = loss_sum / self.eval_dataloader.__len__()
            pickle.dump(accs, open("default_sm.pkl","wb"))
            return acc_mean, loss_mean, rewards
