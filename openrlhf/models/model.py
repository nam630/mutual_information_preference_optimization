...
def _get_hidden_size(config) -> int:
    ...
    raise ValueError(
        ...
        f"Available attrs: {[a for a in vars(config) if 'hidden' in a.lower() or 'size' in a.lower()]}"
    )
        ...
        if dschf is not None:
            ...
            with deepspeed.zero.GatheredParameters([value_head.weight], modifier_rank=0):
                if torch.distributed.get_rank() == 0:
                    value_head.weight.data.normal_(mean=0.0, std=1 / (_get_hidden_size(config) + 1))
        else:
            value_head.weight.data.normal_(mean=0.0, std=1 / (_get_hidden_size(config) + 1))
...
def _get_reward_model(base_pretrained_model, base_llm_model, value_head_prefix="score", packing_samples=False):
    class RewardModel(base_pretrained_model):
        ...
        def __init__(self, config: AutoConfig):
            ...
            setattr(self, value_head_prefix, nn.Linear(_get_hidden_size(config), 1, bias=False))
...
def get_llm_for_multi_class_regression(
    model_name_or_path: str,
    model_type: str,
    *,
    bf16=True,
    ...
    **kwargs,
    ...
    base_pretrained_class = base_class.__base__
    if model_type == "reward":
        cls_class = _get_reward_multi_class_model(base_pretrained_class, base_class, value_head_prefix, packing_samples)
    else:
        cls_class = _get_critic_model(base_pretrained_class, base_class, value_head_prefix, packing_samples)
...
def _get_reward_multi_class_model(base_pretrained_model, base_llm_model, value_head_prefix="score", packing_samples=False):
    class RewardModel(base_pretrained_model):
        supports_gradient_checkpointing = True
        ...
        def __init__(self, config: AutoConfig):
            ...
            self.value_head_prefix = value_head_prefix
            setattr(self, value_head_prefix, nn.Linear(_get_hidden_size(config), 3, bias=False))
        ...
        def forward(
            ...
            if self.packing_samples:
                ...
                eos_indices = packed_seq_lens.cumsum(dim=0) - 1
                if ring_attn_group is not None:
                    reward = all_gather(values, ring_attn_group)
                    if pad_sequence:
                        ring_attn_size = torch.distributed.get_world_size(ring_attn_group)
                        pad_len = (ring_attn_size - reward.shape[1] % ring_attn_size) % ring_attn_size
                        # Since padding was applied at the end during packing, the position of the EOS (End Of Sequence) needs to be corrected.
                    ...
                    reward = values
                reward = reward[:, eos_indices, :]
                reward = reward.permute(1, 0, 2).squeeze(1)
            else:
                eos_indices = attention_mask.size(1) - 1 - attention_mask.long().fliplr().argmax(dim=1, keepdim=True)
                # gather last token per batch across seq_len
                reward = values.gather(dim=1, index=eos_indices.unsqueeze(-1).expand(-1, -1, values.size(-1)))
                reward = reward.squeeze(1)  # [batch, 3]
