...
from openrlhf.datasets import PersonalizedDataset, PromptDataset, SFTDataset
from openrlhf.models import Actor, get_llm_for_sequence_regression
from openrlhf.trainer import PPORMTrainer
...
def train(args):
    ...
    if not args.remote_rm_url:
        reward_model = get_llm_for_sequence_regression(
            ...
            bf16=False,
            load_in_4bit=args.load_in_4bit,
            lora_rank=args.lora_rank, # needs to make the rm trainable
            lora_alpha=args.lora_alpha,
            target_modules=args.target_modules,
            lora_dropout=args.lora_dropout,
            ds_config=strategy.get_ds_train_config(is_actor=False),
    ...
    if args.pets:
        ...
        eval_dataset = PersonalizedDataset(eval_data, tokenizer, strategy, input_template=args.input_template)
    else:
    ...
    prompts_dataset = PersonalizedDataset(prompts_data, tokenizer, strategy, input_template=args.input_template)
    ...
    if args.pets:
        eval_dataloader = strategy.setup_dataloader(
            eval_dataset, args.micro_train_batch_size, True, True
        )
    if args.pretrain_data:
    ...
    reward_optim = strategy.create_optimizer(
        reward_model, lr=args.critic_learning_rate, betas=args.adam_betas, weight_decay=args.l2
    )
    reward_scheduler = get_scheduler(
            "cosine_with_min_lr",
            reward_optim,
            num_warmup_steps=math.ceil(max_steps * args.lr_warmup_ratio),
            num_training_steps=max_steps,
    ...
    (
        ...
        (reward_model, reward_optim, reward_scheduler),
        initial_model,
    ) = strategy.prepare(
        (actor, actor_optim, actor_scheduler),
        (critic, critic_optim, critic_scheduler),
    ...
    if args.pets:
        trainer.fit(args, prompts_dataloader, pretrain_dataloader, consumed_samples, num_update_steps_per_episodes, eval_dataloader)
    else:
...
if __name__ == "__main__":
    ...
    # Checkpoint
    parser.add_argument("--debug_log_path", type=str, default="temp.pkl")
    ...
    parser.add_argument("--prompt_max_len", type=int, default=15000, help="Max tokens for each prompt")
    parser.add_argument("--generate_max_len", type=int, default=100, help="Max tokens to generate in PPO")
    ...
    parser.add_argument("--train_reward", action="store_true", default=False)
    parser.add_argument("--pets", action="store_true", default=False)
