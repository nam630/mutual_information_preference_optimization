...
def preprocess_data_multiprompts(
    data,
    input_template=None,
    prompt_key=None,
    chosen_key="chosen",
    ...
    if apply_chat_template:
        if prompt_key:
            ...
            if is_dpo:
                ...
                chosen = chosen[len(prompt) :]
                rejected = rejected[len(prompt) :]
                # change to rejected prompt
                rejected_prompt = apply_chat_template([{"role": "user", "content": data["rejected_prompt"]}], tokenize=False, add_generation_prompt=True)
    else:
        if prompt_key:
            prompt = data[prompt_key]
            if input_template:
    ...
    return prompt, chosen, rejected, margin, rejected_prompt
...
class CPCDataset(Dataset):
    ...
    def __init__(
        ...
        self.rejects = processed_dataset["reject"]
        self.extras = processed_dataset["extra"]
        self.rejected_prompts = processed_dataset["rejected_prompt"]
        self.rejected_extras = processed_dataset["rejected_extra"]

    def process_data(self, data):
        prompt, chosen, reject, margin, rejected_prompt = preprocess_data_multiprompts(
            data,
            self.input_template,
            self.prompt_key,
            self.chosen_key,
        ...
        if self.is_dpo:
            ...
            rejected_prompt_token = self.tokenizer(
                rejected_prompt,
                max_length=self.max_length,
                padding=False,
                truncation=True,
            ...
            rejected_prompt_ids_len = rejected_prompt_token["attention_mask"].int().sum().item()

            # Filter the sample whose length is greater than max_length (2 for answer length)
            if prompt_ids_len >= self.max_length - 2 or rejected_prompt_ids_len >= self.max_length - 2:
                prompt = None
        ...
        return {
            ...
            "rejected_extra": rejected_prompt_ids_len,
        }
    ...
    def __getitem__(self, idx):
        prompt, chosen, reject, extra, rejected_prompt, rejected_extra = self.prompts[idx], self.chosens[idx], self.rejects[idx], self.extras[idx], self.rejected_prompts[idx], self.rejected_extras[idx]
        
        # chosen response paired with rejected prompt
        chosen_only = (rejected_prompt + chosen).rstrip("\n")
        if not chosen_only.endswith(self.tokenizer.eos_token):
            chosen_only += " " + self.tokenizer.eos_token
        chosen_only_token = self.tokenizer(
            chosen_only,
            max_length=self.max_length,
            padding=False,
            truncation=True,
        ...
        # rejected response paired with rejected prompt
        reject_only = (rejected_prompt + reject).rstrip("\n")
        if not reject_only.endswith(self.tokenizer.eos_token):
            reject_only += " " + self.tokenizer.eos_token

        reject_only_token = self.tokenizer(
            reject_only,
            max_length=self.max_length,
            padding=False,
            truncation=True,
        ...
        # prompt + chosen response concatenation
        chosen = (prompt + chosen).rstrip("\n")
        if not chosen.endswith(self.tokenizer.eos_token):
            chosen += " " + self.tokenizer.eos_token
        chosen_token = self.tokenizer(
        ...
        # prompt + rejected response concatenation
        reject = (prompt + reject).rstrip("\n")
        if not reject.endswith(self.tokenizer.eos_token):
            reject += " " + self.tokenizer.eos_token
        ...
        chosen_only_token["input_ids"][0][-1] = self.tokenizer.eos_token_id
        reject_only_token["input_ids"][0][-1] = self.tokenizer.eos_token_id
        chosen_only_token["attention_mask"][0][-1] = True
        reject_only_token["attention_mask"][0][-1] = True
        ...
        return (
            ...
            reject_token["input_ids"],
            reject_token["attention_mask"],
            chosen_only_token["input_ids"],
            chosen_only_token["attention_mask"],
            reject_only_token["input_ids"],
            reject_only_token["attention_mask"],
            extra,
            rejected_extra,
        )
    ...
    def collate_fn(self, item_list):
        ...
        for chosen_id, chosen_mask, reject_id, rejects_mask, chosen_only_id, chosen_only_mask, reject_only_id, reject_only_mask, extra, rejected_extra in item_list:
            chosen_ids.append(chosen_id)
            chosen_masks.append(chosen_mask)
            reject_ids.append(reject_id)
            rejects_masks.append(rejects_mask)
            
            chosen_only_ids.append(chosen_only_id)
            chosen_only_masks.append(chosen_only_mask)
            reject_only_ids.append(reject_only_id)
            reject_only_masks.append(reject_only_mask)

            extras.append(extra)
            rejected_extras.append(extra)
        ...
        chosen_only_ids = zero_pad_sequences(chosen_only_ids, side=padding_side, value=self.tokenizer.pad_token_id)
        chosen_only_masks = zero_pad_sequences(chosen_only_masks, side=padding_side)
        reject_only_ids = zero_pad_sequences(reject_only_ids, side=padding_side, value=self.tokenizer.pad_token_id)
        rejects_only_masks = zero_pad_sequences(rejects_only_masks, side=padding_side)
        return chosen_ids, chosen_masks, reject_ids, rejects_masks, chosen_only_ids, chosen_only_masks, reject_only_ids, reject_only_masks, extras, rejected_extras
    ...
    def packing_collate_fn(self, item_list):
        ...
        chosen_only_ids = []
        chosen_only_att_masks = []
        chosen_only_seq_lens = []
        
        rejected_only_ids = []
        rejected_only_att_masks = []
        rejected_only_seq_lens = []
        ...
        for chosen_id, chosen_mask, reject_id, rejects_mask, chosen_only_id, chosen_only_mask, reject_only_id, reject_only_mask, extra, rejected_extra in item_list:
            ...
            chosen_seq_lens.append(len(chosen_id.flatten()))
            extras.append(extra)
            rejected_extras.append(rejected_extra)

            rejected_ids.append(reject_id.flatten())
            rejected_att_masks.append(torch.full_like(reject_id.flatten(), index + len(item_list)))
            rejected_seq_lens.append(len(reject_id.flatten()))

            chosen_only_ids.append(chosen_only_id.flatten())
            chosen_only_att_masks.append(torch.full_like(chosen_only_id.flatten(), index))
            chosen_only_seq_lens.append(len(chosen_only_id.flatten()))

            rejected_only_ids.append(reject_only_id.flatten())
            rejected_only_att_masks.append(torch.full_like(reject_only_id.flatten(), index + len(item_list)))
            rejected_only_seq_lens.append(len(reject_only_id.flatten()))
        ...
        only_packed_input_ids = torch.cat(chosen_only_ids + rejected_only_ids, dim=0).unsqueeze(0)
        only_packed_attention_masks = torch.cat(chosen_only_att_masks + rejected_only_att_masks, dim=0).unsqueeze(0)
        only_packed_seq_lens = chosen_only_seq_lens + rejected_only_seq_lens
        ...
        if self.multiple_of > 1 and only_packed_input_ids.numel() % self.multiple_of != 0:
            only_padding_len = self.multiple_of - (only_packed_input_ids.numel() % self.multiple_of)
            only_packed_input_ids = F.pad(only_packed_input_ids, (0, padding_len), value=self.tokenizer.pad_token_id)
            only_packed_attention_masks = F.pad(only_packed_attention_masks, (0, padding_len), value=0)

        return packed_input_ids, packed_attention_masks, packed_seq_lens, only_packed_input_ids, only_packed_attention_masks, only_packed_seq_lens, extras, rejected_extras
