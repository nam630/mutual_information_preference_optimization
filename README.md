...
```deepspeed --module openrlhf.cli.train_ppo   --pretrain Qwen/Qwen2.5-3B-Instruct     --reward_pretrain Qwen/Qwen2.5-1.5B-Instruct  --save_path ./checkpoint/Qwen1.5B --save_steps 10   --logging_steps 1   --eval_steps -1    --micro_train_batch_size 1   --train_batch_size 128 --micro_rollout_batch_size 2   --rollout_batch_size 256  --max_epochs 1   --prompt_max_len 15360 --generate_max_len 256   --zero_stage 2   --bf16   --actor_learning_rate 5e-7   --critic_learning_rate 9e-6  --prompt_data json@ {path to the file directory, where file is saved as a train.jsonl}  --input_key context_messages   --apply_chat_template   --max_samples 100000   --normalize_reward   --flash_attn   --gradient_checkpointing   --use_wandb 173ea73581c8f47fb8aa18a4b33040f44e4ebdd6 --save_hf_ckpt --num_episodes 1 --debug_log_path tmp.pkl --model_save_path tmp_model ```

The dataset requires the following fields:
`context, context_messages, rejected, chosen, reward_prompt, train_prompt, train_rejected, train_chosen, eval_prompt, eval_rejected, eval_chosen`
`reward_prompt, rejected, chosen` are used for reward training
`train_prompt, train_rejected, train_chosen` are used for computing the RL signal under the current reward model (but you can use the same `reward_prompt, rejected, chosen`)
`eval_prompt, eval_rejected, eval_chosen` are used for reward model evaluation (but you can use the same `reward_prompt, rejected, chosen` - not necessary for PLUS, this is just an additional check for the reward model evaluation)

`context, context_messages` are used by the summarizer, and the context (prompt to the summarizer) ends with "Start your response with ##Summary" so the summary output can be easily extracted and used to condition the reward model. 

For an example training dataset, look at `train.jsonl` (which can be downloaded from gdrive https://drive.google.com/file/d/1a_rtzvGsNTtn-Numnn9zdXonj37Yp2ud/view?usp=sharing) 
